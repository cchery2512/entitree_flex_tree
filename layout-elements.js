...
export const layoutElements = (tree, rootId, direction = 'TB') => {
  ...
  entitreeEdges.forEach((edge) => {
    ...
    const newEdge = {};
    // Crear un key único basado en source-target
    const edgeKey = `${sourceNode}-${targetNode}`;
    // Incrementar el contador de edges entre estos dos nodos
    if (!edgeCountMap[edgeKey]) {
      edgeCountMap[edgeKey] = 0;
    }
    edgeCountMap[edgeKey]++;
    newEdge.id = `e${sourceNode}${targetNode}-${edgeCountMap[edgeKey]}`;
    //console.log(newEdge);
